...
| Target | Command |
|--------|---------|
| Pipeline directory (recommended) | `mthds-agent <runner> run bundle <bundle-dir>/` |
| Specific pipe in a directory | `mthds-agent <runner> run bundle <bundle-dir>/ --pipe my_pipe` |
| Bundle file directly | `mthds-agent <runner> run bundle bundle.mthds -L <bundle-dir>/` |
| Pipe by code from library | `mthds-agent <runner> run bundle my_pipe` |
...
```bash
mthds-agent <runner> inputs bundle bundle.mthds
```
...
**No inputs required**: If `mthds-agent <runner> inputs bundle <file>.mthds` returns an empty `inputs` object (`{}`), inputs are ready — skip to Step 3.
...
|------|---------|----------|
| **Dry run + mock inputs** | `mthds-agent <runner> run bundle <bundle-dir>/ --dry-run --mock-inputs` | Quick structural validation, no real data needed, or inputs not ready |
| **Dry run with real inputs** | `mthds-agent <runner> run bundle <bundle-dir>/ --dry-run` | Validate input shapes without making API calls (auto-detects `inputs.json`) |
| **Full run** | `mthds-agent <runner> run bundle <bundle-dir>/` | Production execution (auto-detects `inputs.json`) |
| **Full run inline** | `mthds-agent <runner> run bundle <bundle-dir>/ --inputs '{"theme": ...}'` | Quick execution with inline JSON inputs |
| **Full run without graph** | `mthds-agent <runner> run bundle <bundle-dir>/ --no-graph` | Execute without generating graph visualization |
| **Full run with memory** | `mthds-agent <runner> run bundle <bundle-dir>/ --with-memory` | When piping output to another method |
...
```bash
# Inline JSON
mthds-agent <runner> run bundle <bundle-dir>/ --inputs '{"theme": {"concept": "native.Text", "content": {"text": "nature"}}}'
